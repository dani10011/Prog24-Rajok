    ...
    public class RoomEntryRequestService : IRoomEntryRequestService
    {
        ...
        private RoomEntryRequestResponse MapToResponse(RoomEntryRequest request)
        {
            return new RoomEntryRequestResponse
            {
                ...
                StudentEmail = request.Student?.User?.Email ?? "unknown@email.com",
                InstructorId = request.Instructor_Id,
                InstructorName = request.Instructor?.User?.Name ?? "Unknown Instructor",
                RoomId = request.Room_Id,
                RoomNumber = request.Room?.Room_Number ?? "Unknown Room",
                BuildingName = request.Room?.Building?.Name ?? "Unknown Building",
