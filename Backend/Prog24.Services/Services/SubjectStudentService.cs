...
namespace Prog24.Services.Services
{
    public class SubjectStudentService : ISubjectStudentService
    {
        ...
        public SubjectStudentService(AppDbContext dbContext)
        {
            _dbContext = dbContext;
        }

        public async Task<List<SubjectStudent>> GetSubjectStudents()
        {
            var result = await _dbContext.Subject_student.ToListAsync();
            return result;
        }
    }
}
